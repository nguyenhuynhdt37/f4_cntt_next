...
import { updateSlide, getSlideById, ISlideUpdate, ISlide } from '@/api/axios/slides';

interface EditSlideFormProps {
    slideId: number;
}

export default function EditSlideForm({ slideId }: EditSlideFormProps) {
    ...
    const [formData, setFormData] = useState<{
        ...
        currentOverlayImageUrl: string | null;
    }>({
        title: '',
        description: '',
        image: null,
        ...
        const fetchSlide = async () => {
            try {
                ...
                setFormData({
                    ...
                    currentImageUrl: slide.image || null,
                    currentOverlayImageUrl: slide.overlayImage || null,
                });
                ...
                if (slide.overlayImage) {
                    setOverlayImagePreview(slide.overlayImage);
                }
            } catch (error) {
                console.error('Lỗi khi tải thông tin slide:', error);
                setErrors({ form: 'Không thể tải thông tin slide. Vui lòng thử lại sau.' });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.image) {
                slideData.image = formData.image;
            }
            ...
            setErrors({
                form: 'Có lỗi xảy ra khi cập nhật slide. Vui lòng thử lại sau.'
            });
        } finally {
        ...
        <div className="max-w-4xl mx-auto">
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900">Chỉnh sửa slide</h1>
                </div>
            </div>
                    ...
                    <div className="p-6 space-y-6">
                                        ...
                                        {imagePreview ? (
                                            ...
                                            <>
                                                ...
                                                <p className="text-sm text-gray-500 text-center mt-2">
                                                    Click để tải lên hình ảnh slide mới<br />
                        ...
                        <div>
                            ...
                            <div className="flex flex-col space-y-2">
                                ...
                                {overlayImagePreview && formData.overlayImage && (
                                    <button
                                        type="button"
                                        className="text-sm text-red-600 hover:text-red-800 self-start"
                                        onClick={() => {
                                            // Reset to original overlay image if available
                                            if (formData.currentOverlayImageUrl) {
                                                setOverlayImagePreview(formData.currentOverlayImageUrl);
                                            } else {
